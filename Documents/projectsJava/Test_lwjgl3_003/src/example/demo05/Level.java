...
*/
	...
	if(movementVector.length() > 0) {
            ...
            Vector2f oldPos2 = new Vector2f(oldPos.x, oldPos.z);
            Vector2f newPos2 = new Vector2f(newPos.x, newPos.z);
      ...
      /*      for(Door door : doors) {
		...
		Vector2f doorPos2f = new Vector2f(doorPos3f.x, doorPos3f.z);
		collisionVector = collisionVector.mul(rectCollide(oldPos2, newPos2, objectSize, doorPos2f, doorSize));
		}
*/
	}
		
	return new Vector3f(collisionVector.x, 0, collisionVector.y);
    }
	...
	for(Door door : doors) {
            Vector2f doorSize = door.getDoorSize();
            Vector3f doorPos3f = door.getTransform().getTranslation();
            Vector2f doorPos2f = new Vector2f(doorPos3f.x, doorPos3f.y);
	...
	if(hurtMonsters) {
		...
		Vector2f monsterPos2f = new Vector2f(monsterPos3f.x, monsterPos3f.z);
...
*/
	...
	public Vector2f lineIntersectRect(Vector2f lineStart, Vector2f lineEnd, Vector2f rectPos, Vector2f rectSize)
	{
		...
		Vector2f collisionVector = lineIntersect(lineStart, lineEnd, rectPos, new Vector2f(rectPos.x + rectSize.x, rectPos.y));
		result = findNearestVector2f(result, collisionVector, lineStart);

		collisionVector = lineIntersect(lineStart, lineEnd, rectPos, new Vector2f(rectPos.x, rectPos.y + rectSize.y));
		result = findNearestVector2f(result, collisionVector, lineStart);

		collisionVector = lineIntersect(lineStart, lineEnd, new Vector2f(rectPos.x, rectPos.y + rectSize.y), rectPos.add(rectSize));
		result = findNearestVector2f(result, collisionVector, lineStart);

		collisionVector = lineIntersect(lineStart, lineEnd, new Vector2f(rectPos.x + rectSize.x, rectPos.y), rectPos.add(rectSize));
		result = findNearestVector2f(result, collisionVector, lineStart);
